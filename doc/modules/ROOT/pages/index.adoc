:github-address: https://github.com/hazelcast-guides/striiim-cdc
...
To run Striim evaluation version, run the following docker commands:
...
That is the excerpt from Striim DockerHub Page if you experience an expired license key issue:

----
Note: This docker image is an evaluation version which comes with a trial license valid for
15 days and single node. Any docker image older than 15 days would not work. We try and keep
pushing our latest releases on docker hub. But in case, you don't see an image within 15
days or if you need extended license, please contact support@striim.com
----
...
[source,bash]
----
 $ git clone https://github.com/hazelcast-guides/striiim-cdc.git
 $ cd pojo
 $ mvn clean install
----
...
In the previous sections, we installed required software for the sample application. We now will install OracleHazelcastCDC app through Striim dashboard. There are two ways to install OracleHazelcastCDC App. We recommend to use `+Using TQL file (Quick Setup)+`
