:github-address: https://github.com/hazelcast-guides/striim-cdc
...
15 days and a single node. Any docker image older than 15 days would not work. We try and keep
pushing our latest releases on DockerHub. But in case, you don't see an image within 15
days or if you need an extended license, please contact support@striim.com
----
...
In the previous sections, we installed the required software for the sample application. We'll now install OracleHazelcastCDC app through Striim dashboard. There are two ways to install OracleHazelcastCDC App. We recommend to use `+Using TQL file (Quick Setup)+`
...
. Change `+{ORACLE_DB_ADDRESS}+` and `+{HZ_IP_ADDRESS}+` placeholders with your HOST IP addresses at `+config/OracleHazelcastCDC.tql+`. You can modify these values before deploying the app as well.
